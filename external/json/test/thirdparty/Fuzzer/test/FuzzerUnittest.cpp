...
U.push_back('d');
EXPECT_EQ("81fe8bfe87576c3ecb22426f8e57847382917acf",
fuzzer::Hash(U)
);
}
...
TestEraseBytes(&MutationDispatcher::Mutate_EraseBytes,
200);
}
TEST(FuzzerMutate, EraseBytes2
) {
TestEraseBytes(&MutationDispatcher::Mutate,
2000);
}
...
TestInsertByte(&MutationDispatcher::Mutate_InsertByte,
1 << 15);
}
TEST(FuzzerMutate, InsertByte2
) {
TestInsertByte(&MutationDispatcher::Mutate,
1 << 17);
}
...
TestInsertRepeatedBytes(&MutationDispatcher::Mutate,
300000);
}
...
TestChangeByte(&MutationDispatcher::Mutate_ChangeByte,
1 << 15);
}
TEST(FuzzerMutate, ChangeByte2
) {
TestChangeByte(&MutationDispatcher::Mutate,
1 << 17);
}
...
TestChangeBit(&MutationDispatcher::Mutate_ChangeBit,
1 << 16);
}
TEST(FuzzerMutate, ChangeBit2
) {
TestChangeBit(&MutationDispatcher::Mutate,
1 << 18);
}
...
TestShuffleBytes(&MutationDispatcher::Mutate_ShuffleBytes,
1 << 16);
}
TEST(FuzzerMutate, ShuffleBytes2
) {
TestShuffleBytes(&MutationDispatcher::Mutate,
1 << 20);
}
...
TestCopyPart(&MutationDispatcher::Mutate_CopyPart,
1 << 10);
}
TEST(FuzzerMutate, CopyPart2
) {
TestCopyPart(&MutationDispatcher::Mutate,
1 << 13);
}
...
TestAddWordFromDictionary(&MutationDispatcher::Mutate,
1 << 15);
}
...
TestAddWordFromDictionaryWithHint(&MutationDispatcher::Mutate,
1 << 10);
}
...
TestChangeASCIIInteger(&MutationDispatcher::Mutate,
1 << 15);
}
...
TestChangeBinaryInteger(&MutationDispatcher::Mutate,
1 << 15);
}
...
'a', 'b'}));
EXPECT_EQ("eHk=", Base64({
'x', 'y'}));
EXPECT_EQ("YWJj", Base64({
'a', 'b', 'c'}));
EXPECT_EQ("eHl6", Base64({
'x', 'y', 'z'}));
EXPECT_EQ("YWJjeA==", Base64({
'a', 'b', 'c', 'x'}));
EXPECT_EQ("YWJjeHk=", Base64({
'a', 'b', 'c', 'x', 'y'}));
EXPECT_EQ("YWJjeHl6", Base64({
'a', 'b', 'c', 'x', 'y', 'z'}));
}
...
i++)
C.
AddToCorpus(Unit{static_cast<uint8_t>(i)},
0);
