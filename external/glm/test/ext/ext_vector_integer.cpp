...
namespace prevMultiple {
    ...
    int run() {
        ...
        for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<T>); i < n; ++i) {
            ...
            glm::vec<L, T> const Result1 = glm::prevMultiple(glm::vec<L, T>(Data[i].Source),
                                                             glm::vec<L, T>(Data[i].Multiple));
...
namespace nextMultiple {
    ...
    int run() {
        ...
        for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<T>); i < n; ++i) {
            glm::vec<L, T> const Result0 = glm::nextMultiple(glm::vec<L, T>(Data[i].Source),
                                                             glm::vec<L, T>(Data[i].Multiple));
            Error += glm::vec<L, T>(Data[i].Return) == Result0 ? 0 : 1;
...
namespace findNSB {
    ...
    int run() {
        ...
        for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<T>); i < n; ++i) {
            glm::vec<L, int> const Result0 = glm::findNSB<L, T, glm::defaultp>(glm::vec<L, T>(Data[i].Source),
                                                                               glm::vec<L, int>(
                                                                                       Data[i].SignificantBitCount));
