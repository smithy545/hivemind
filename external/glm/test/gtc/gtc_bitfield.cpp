...
*/
    ...
    int perf_old_u8vec4(std::vector<glm::uint32> &Result) {
        ...
        for (glm::size_t k = 0; k < 10000; ++k)
            for (glm::size_t j = 0; j < 256; ++j)
                for (glm::size_t i = 0; i < 256; ++i)
                    Error += Result[j * 256 + i] ==
                             glm::bitfieldInterleave(glm::uint8(i), glm::uint8(j), glm::uint8(i), glm::uint8(j)) ? 0
                                                                                                                 : 1;
    ...
    int perf_new_u8vec4(std::vector<glm::uint32> &Result) {
        ...
        for (glm::size_t k = 0; k < 10000; ++k)
            for (glm::size_t j = 0; j < 256; ++j)
                for (glm::size_t i = 0; i < 256; ++i)
                    Error += Result[j * 256 + i] ==
                             bitfieldInterleave_u8vec4(glm::uint8(i), glm::uint8(j), glm::uint8(i), glm::uint8(j)) ? 0
                                                                                                                   : 1;
    ...
    int perf() {
        ...
        for (glm::size_t j = 0; j < 256; ++j)
            for (glm::size_t i = 0; i < 256; ++i)
                Result_u8vec4[j * 256 + i] = glm::bitfieldInterleave(glm::uint8(i), glm::uint8(j), glm::uint8(i),
                                                                     glm::uint8(j));
