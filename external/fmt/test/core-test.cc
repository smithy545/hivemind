...
push_back(str);
store.
push_back(std::cref(str)
);
store.
push_back(fmt::string_view{str}
);
str[0] = 'X';
...
push_back (copy_throwable());

} catch (...) {
}
EXPECT_EQ(fmt::vformat("{}", store),
"foo");
}
...
EXPECT_EQ(std::strlen(str), string_view(str)
.

size()

);
EXPECT_LT(std::strlen(str),
sizeof(str));
}
...
.
compare(string_view("foo")
), 0);
EXPECT_GT(string_view("fop")
.
compare(string_view("foo")
), 0);
EXPECT_LT(string_view("foo")
.
compare(string_view("fop")
), 0);
EXPECT_GT(string_view("foo")
.
compare(string_view("fo")
), 0);
EXPECT_LT(string_view("fo")
.
compare(string_view("foo")
), 0);
...
FMT_BEGIN_NAMESPACE
        template<>
        struct formatter<enabled_formatter> {
...
FMT_BEGIN_NAMESPACE
        template<>
        struct formatter<convertible_to_int> {
            auto parse(format_parse_context &ctx) -> decltype(ctx.begin()) {
                return ctx.begin();
            }
        ...
        template<>
        struct formatter<convertible_to_c_string> {
            auto parse(format_parse_context &ctx) -> decltype(ctx.begin()) {
                return ctx.begin();
            }
...
EXPECT_EQ(fmt::format("{}", convertible_to_int()),
"foo");
EXPECT_EQ(fmt::format("{}", convertible_to_c_string()),
"bar");
}
...
EXPECT_EQ(to_string_view(my_string<char>("42")),
"42");
EXPECT_EQ(to_string_view(my_string<wchar_t>(L"42")),
L"42");
EXPECT_EQ(to_string_view(QString(L"42")),
L"42");
...
using namespace my_ns;
using namespace FakeQt;
EXPECT_EQ(fmt::format(my_string<char>("{}"), 42),
"42");
EXPECT_EQ(fmt::format(my_string<wchar_t>(L"{}"), 42),
L"42");
EXPECT_EQ(fmt::format(QString(L"{}"), 42),
L"42");
EXPECT_EQ(fmt::format(QString(L"{}"), my_string<wchar_t>(L"42")),
L"42");
EXPECT_EQ(fmt::format(my_string<wchar_t>(L"{}"), QString(L"42")),
L"42");
}
