...
EXPECT_EQ(fmt::internal::bit_cast<uint64_t>(s),
42ull);
...
EXPECT_EQ(static_cast<size_t>(0), buffer.

size()

);
EXPECT_EQ(123u, buffer.
...
int mem[20];
mem[7] = 0xdead;
EXPECT_CALL(alloc, allocate(20)
).
WillOnce(Return(mem)
);
buffer.grow(20);
EXPECT_EQ(20u, buffer.
...
EXPECT_CALL(alloc, allocate(size)
).
WillOnce(Return(&mem)
);
buffer2.
...
WillOnce(Return(&mem[0])
);
buffer.
resize(size);
std::fill(&buffer[0], &buffer[0]
+ size, 'x');
}
std::vector<char> mem2(2 * size);
{
EXPECT_CALL(alloc, allocate(2 * size)
).
WillOnce(Return(&mem2[0])
);
std::exception e;
...
EXPECT_EQ(L"\x043B\x043E\x0448\x0430\x0434\x043A\x0430", u.
...
EXPECT_EQ(L"\xD801\xDC37", fmt::internal::utf8_to_utf16("𐐷").
...
EXPECT_EQ(L"123456", fmt::internal::utf8_to_utf16("123456").
...
fmt::format_system_error(message,
EDOM, "test");
...
fmt::format_system_error(message,
EDOM, fmt::string_view(nullptr, max_size));
...
EXPECT_EQ(fmt::format("test: {}", get_system_error(EDOM)), e
.
...
fmt::format_system_error(message,
EDOM, "test error");
EXPECT_EQ(to_string(message), error
.
...
EXPECT_STREQ("abc", string_view("abc").

data()

);
EXPECT_EQ(3u, string_view("abc").
...
fmt::internal::use_grisu<long double>()
...
for (
int i = 0;
i < 100; ++i) writer.write(1.23456789);
}
...
EXPECT_THROW_MSG(TestFormat<21>::format("{21}"), format_error,
                 "argument index out of range");
enum {
    max_packed_args = fmt::internal::max_packed_args
};
...
char buffer[16] = {};
fmt::format_to(fmt::internal::make_checked(buffer, 16),
"{: =+}", 42.0);
...
EXPECT_EQ("42", format("{0}",

Answer()

));
EXPECT_EQ("0042", format("{:04}",
...
EXPECT_EQ("42", fmt::format_int(42).

str()

);
EXPECT_EQ(2u, fmt::format_int(42).
...
EXPECT_EQ(3u, fmt::format_int(-42).

size()

);
EXPECT_EQ("42", fmt::format_int(42ul).

str()

);
EXPECT_EQ("-42", fmt::format_int(-42l).

str()

);
EXPECT_EQ("42", fmt::format_int(42ull).

str()

);
EXPECT_EQ("-42", fmt::format_int(-42ll).
...
str(), fmt::format_int(max_value<int64_t>())
...
args.
emplace_back(fmt::internal::make_arg<ctx>(42)
);
args.
emplace_back(fmt::internal::make_arg<ctx>("abc1")
);
args.
emplace_back(fmt::internal::make_arg<ctx>(1.5f)
);
...
join(v1, v1,
", ")));
EXPECT_EQ("(001, 002, 003)", format("({:03})",
join(v1, v1
+ 3, ", ")));
...
EXPECT_EQ(L"(1, 2, 3)", format(L"({})",
join(v1, v1
+ 3, L", ")));
EXPECT_EQ("1, 2, 3", format("{0:{1}}",
join(v1, v1
+ 3, ", "), 1));
...
join(v1,
", ")));
EXPECT_EQ("(+01.20, +03.40)", format("({:+06.2f})",
join(v2,
", ")));
#endif
}
...
EXPECT_EQ("foo", fmt::format(FMT_STRING("{}"),
...
EXPECT_EQ("42", fmt::format(FMT_STRING("{}"),
...
std::stringstream s;
fmt::format_to(std::ostream_iterator<char>(s),
"{}", 42);
EXPECT_EQ("42", s.
...
char buf[10] = {};
fmt::format_to_n(test_output_iterator{buf},
10, "{}", 42);
EXPECT_STREQ(buf,
"42");
}
...
struct test_format_specs_handler {
    enum Result {
        NONE, PLUS, MINUS, SPACE, HASH, ZERO, ERROR
    };
    Result res = NONE;
...
std::wstring w;
fmt::vformat_to(std::back_inserter(w),
L"{}", wargs);
EXPECT_EQ(L"42", w);
w.
...
template<typename T>
static std::string FmtToString(const T &t) {
    return fmt::format(FMT_STRING("{}"), t);
}

TEST(FormatTest, FmtStringInTemplate
) {
EXPECT_EQ(FmtToString(1),
"1");
EXPECT_EQ(FmtToString(0),
"0");
}
...
struct mychar {
    ...
    template<typename T>
    mychar(T val) : value(static_cast<int>(val)) {}
...
FMT_BEGIN_NAMESPACE
        template<>
        struct is_char<mychar> : std::true_type {
        };
FMT_END_NAMESPACE
...
template<typename S>
std::string from_u8str(const S &str) {
...
EXPECT_EQ(fmt::internal::count_code_points(result),
4);
EXPECT_EQ(result
.
