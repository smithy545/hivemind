...
fmt::format_to(fmt::internal::make_checked(buffer, 32), prepared,
2);
EXPECT_EQ(std::string("42"), buffer
);
wchar_t wbuffer[32] = {0};
const auto wprepared = fmt::compile<int>(L"4{}");
fmt::format_to(fmt::internal::make_checked(wbuffer, 32), wprepared,
2);
...
auto f = fmt::compile<int>("{:10}");
EXPECT_EQ(fmt::formatted_size(f, 42),
10);
}
...
EXPECT_EQ(fmt::format(f, formattable()),
"foo");
}
