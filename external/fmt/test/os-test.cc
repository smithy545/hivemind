...
EXPECT_EQ(ERROR_INVALID_PARAMETER, u.
convert(fmt::wstring_view(0, 1)
));
EXPECT_EQ(ERROR_INVALID_PARAMETER,
u.
convert(fmt::wstring_view(L"foo", INT_MAX + 1u)
));
}
...
fmt::internal::format_windows_error(actual_message,
ERROR_FILE_EXISTS,
"test");
...
FORMAT_MESSAGE_IGNORE_INSERTS,
0, static_cast
<DWORD>(provisioning_not_allowed),
MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
reinterpret_cast
<LPWSTR>(&message),
0, 0) == 0) {
...
fmt::internal::format_windows_error(message,
ERROR_FILE_EXISTS, "test error");
...
EXPECT_EQ(ERROR_FILE_EXISTS, error.
...
fmt::memory_buffer out;
fmt::internal::format_windows_error(out,
ERROR_FILE_EXISTS, "test error");
out.push_back('\n');
